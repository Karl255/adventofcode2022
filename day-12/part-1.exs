defmodule H do
  ...
  def find_end(nodes, ending, {_, width, height} = map, visited, steps \\ 0) do
    if Enum.member?(nodes, ending) do
      steps
    else
        ...
        Enum.flat_map(nodes, fn {x, y} ->
          ...
          [
            if x + 1 < width do
              ...
              if next_height - 1 <= at_height and not MapSet.member?(visited, next_coords) do
                next_coords
              end
            end,
            if x - 1 >= 0 do
      ...
      find_end(next_nodes, ending, map, visited, steps + 1)
    end
  end
end
...
start = Enum.find_index(heightmap_list, &(&1 === ?S)) |> H.to_coords(map_width)
...
H.find_end([start], ending, {heightmap, map_width, map_height}, visited)
|> IO.inspect()
